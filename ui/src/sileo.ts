// A tiny toast shim standing in for `@ryu/ui`'s `sileo` toast, which depends on
// shell React context/portals unavailable in the sandbox. Renders a self-contained,
// auto-dismissing toast into a fixed corner container. Same call surface the ported
// page uses: `toast.error("…")` / `toast.success("…")` (the desktop `toast` accepts
// a plain string, so this shim does too).
