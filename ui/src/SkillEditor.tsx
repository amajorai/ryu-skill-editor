...
//   • `@ryu/ui`'s `sileo` toast (shell portal) → the shared frame-local
//     `@ryu/ui/components/sandbox-toast` primitive.
...
export function SkillEditor() {
	...
	const handleDistribute = useCallback(async () => {
		...
		try {
			...
			await distributeSkill(id);
		} catch (error) {
			toast.error(
				`Couldn't use this skill with agents${error instanceof Error ? `: ${error.message}` : "."}`
			);
		} finally {
	...
	const versionSource = useMemo<VersionSource>(
		() => ({
			...
			snapshot: async (label) => {
				...
				if (!(await flush())) {
					throw new Error("Couldn't save the latest edits");
				}
		...
		return (
			<Empty className="h-full">
				<EmptyHeader>
					...
					<EmptyDescription>
						Something went wrong loading it. Check your connection and try
						again.
					</EmptyDescription>
				</EmptyHeader>
				<EmptyContent>
			...
			<div className="flex shrink-0 items-center gap-3 border-b px-4 py-2">
				...
				{isNew ? (
					...
					<>
						...
						<Button
							disabled={saveState === "saving" || distributing}
							onClick={handleDistribute}
							size="sm"
							variant="outline"
						>
							{distributing ? <Spinner className="size-3" /> : null}
							Use with agents
						</Button>
					</>
				)}
